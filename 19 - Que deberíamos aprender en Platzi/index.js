function learn(time, courses) {
  ...
  for (let i = 0; i < courses.length; i++) {
    for (let j = 0; j < courses.length; j++) {
      ...
      const courseTimeI = courses[i];
      const courseTimeJ = courses[j];
      const sumCourses = courseTimeI + courseTimeJ;

      if (i != j && sumCourses <= time && sumCourses > bestTime) {
        bestTime = sumCourses;
        coursesIndex = [i, j];

        if (sumCourses === time) return coursesIndex;
      }
    }
  }

  return coursesIndex.length > 0 ? coursesIndex : null;
}
